...
import sys
sys.path.append('/data2/ABC2/AtlasNet/')
import torch
from AtlasNet.model.model_blocks import PointNet
from torch.utils.data import Dataset
import glob, os
from torch.utils.data.dataloader import DataLoader
from tqdm import tqdm
from ptdec.utils import custom_rand_score
...
FLAGS.add_argument('--num_classes', help='Location of experiments config file')


    
def main():
    args = FLAGS.parse_args()
    p = create_config(args.config_env, args.config_exp, int(args.num_classes))
    print(colored(p, 'red'))
...
#     else:
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        if lowest_loss < best_loss:
            ...
            print('saving to {}'.format(p['scan_model']))
...
#         clustering_stats = custom_rand_score(np.array(predictions[0]['predictions']), 
#                                              np.array(predictions[0]['targets']))
#         print(clustering_stats)     
    ...
    assert(len(predictions_train)==1)
    
    with open(os.path.join(p['scan_dir'], '{}_cluster_outputs_train.pkl'.format(p['num_classes'])), 'wb') as f:
        pickle.dump(predictions_train, f)
        
    with open(os.path.join(p['scan_dir'], '{}_cluster_outputs_val.pkl'.format(p['num_classes'])), 'wb') as f:
        pickle.dump(predictions, f)        
        
    predictions_total = {}
    for pk, pv in predictions[0].items():
        if pk not in ['predictions','targets']:
            continue
        try:
            predictions_total[pk] = pv.append(predictions_train[0][pk])
        except:
            predictions_total[pk] = torch.cat((pv, predictions_train[0][pk]))
            
    clustering_stats = custom_rand_score(np.array(predictions_total['predictions']), 
                                         np.array(predictions_total['targets']))
    predictions_total['rand_score'] = clustering_stats
    with open(os.path.join(p['scan_dir'], '{}_cluster_outputs.pkl'.format(p['num_classes'])), 'wb') as f:
        pickle.dump(predictions_total, f)
