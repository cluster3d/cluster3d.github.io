...
def create_config(config_file_env, config_file_exp, num_classes_override=None):
    ...
    if cfg['setup'] in ['scan', 'selflabel']:
        ...
        cfg['scan_checkpoint'] = os.path.join(scan_dir, '{}_checkpoint.pth.tar'.format(num_classes_override))
        cfg['scan_model'] = os.path.join(scan_dir, '{}_model.pth.tar'.format(num_classes_override))
    ...
    if num_classes_override is not None:
        cfg['num_classes'] = num_classes_override
