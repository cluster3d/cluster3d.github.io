...
from ptdec.utils import target_distribution, custom_rand_score
...
def train(
    ...
    accuracy = custom_rand_score(predicted, actual.cpu().numpy())
    ...
    for epoch in range(epochs):
        ...
        for index, batch in enumerate(data_iterator):
            ...
            loss_value_epoch =  float(loss.item())
            optimizer.zero_grad()
            loss.backward()
        ...
        f=open("{}/training_accuracy.txt".format(output_path),"a")
        f.write(str(epoch)+","+str(round(accuracy, 2))+","+str(round(loss_value_epoch, 2)))
        f.close() 
        ...
        accuracy = custom_rand_score(predicted.cpu().numpy(), actual.cpu().numpy())
